...
<main id="main-container">
                ...
                @if(session('course'))
                    <nav class="flex-shrink-0 mt-3 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                        <div class="alert alert-secondary alert-dismissible alert-success" role="alert">
                            <p class="mb-0">
                                {{ session()->get('course') }} <a class="alert-link" href="{{ route('dashboard') }}">OK</a>!
                            </p>
    ...
    <div class="content">
        ...
        <form class="js-validation" action="{{ route('course.update', ['id' => $course->id]) }}" method="POST">
                    ...
                    <div class="row items-push">
                        ...
                        <div class="col-lg-8 col-xl-5">
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" id="val-username" value="{{ $course->title ?? old('course-title') }}" name="course-title">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" id="val-username"  name="course-code" value="{{ $course->code ?? old('course-code') }}">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" value="{{ $course->unit ?? old('course-unit') }}" class="form-control" id="val-password" name="course-unit">
                            ...
                            <div class="form-floating mb-4">
                                <select class="form-select" id="example-select-floating" name="course-level" aria-label="Select Course Level">
                                    <option value="{{ $course->level }}" @selected(old('course-level'))>{{ $course->level . ' level' }}</option>
                            ...
                            <div class="form-floating mb-4">
                                <select class="form-select" id="example-select-floating" name="course-semester" aria-label="Select Course Semester">
                                    <option value="{{ $course->semester }}" @selected(old('course-semester'))>{{ $course->semester .' Semester' }}</option>
                            ...
                            <div class="form-floating mb-4">
                                <select class="form-select" id="example-select-floating" name="course-department" aria-label="Select Course faculty">

                                    <option selected value="{{ $course->department->id }}">{{ $course->department->name }}</option>

                                    @foreach($departments as $department)
                                        <option value={{ $department->id }} @selected(old('course-department'))>{{ $department->name }}</option>
                                    @endforeach

                                </select>
                                <label for="example-select-floating">Department</label>

                                @if($errors->any('course-faculty'))
